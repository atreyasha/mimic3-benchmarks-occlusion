...
This visualization shows nominal perturbations recorded during occlusion. `normal` on the top-right refers to replacing features of interest marginally by their `normal` or healthy values, which were suggested by medical experts. We shall focus on `normal` occlusion for this interpretation.
