...
def load_test_data_ihm():
    ...
    train_reader = InHospitalMortalityReader(dataset_dir
                                             ...
                                             period_length
                                             =48.0)
    discretizer_header = discretizer.transform(train_reader.read_example(0)
                                               ["X"])[1].split(',')
    cont_channels = [i for (i, x) in enumerate(discretizer_header)
                    if x.find("->") == -1]
    normalizer = Normalizer(fields=cont_channels)
    normalizer.load_params(normalizer_state)
    test_reader = InHospitalMortalityReader(dataset_dir
                                            =os.path.join(data_dir,
                                                          'test'),
                                            listfile
                                            =os.path.join(data_dir,
...
def create_feature_indices(header):
    ...
    for i,head in enumerate(header):
        ...
        if current[0] == "mask":
            feature_indices["presence_"+current[1]
                            .replace(" ","_")] = ["discrete",i]
...
def zero_normal_occlude(feature_indices,data,model,
        ...
        for key in feature_indices.keys():
            ...
            if feature_indices[key][0] == "discrete":
                if "presence_" in key:
                    ...
                    category_id = possible_values[full_name].index(normal_value)
                ...
                to_sub = ((float(normal_values[full_name])-
                           normalizer._means[index])
                          /normalizer._stds[index])
            cols = feature_indices[key][1:]
            occluded_data = data.copy()
...
def occlude(occlusion_type="all",n_iterations=5):
    ...
    res_dict = {"instance":names,"true":labels,"best_score":base_predictions}
    ...
    if occlusion_type != "all":
        ...
        occlusion_type = "zero"
        pd_df_zero = zero_normal_occlude(feature_indices,data,model,
                                         occlusion_type,pd_df.copy(deep=True),
                                         header)
        occlusion_type = "normal-value"
        pd_df_nv = zero_normal_occlude(feature_indices,data,model,
                                       occlusion_type,pd_df.copy(deep=True),
                                       header)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--occlusion-type", type=str, default="all",
                        help="type of occlusion to conduct, either"+
                        " 'zero', 'normal-value', 'inner', 'outer'"+
                        " or 'all'")
    inner_outer = parser.add_argument_group("arguments specific to inner/outer"+
                                            " occlusion")
    inner_outer.add_argument("--n-iterations", type=int, default=5,
                             help="number of permutation iterations for inner"+
                             "/outer occlusion")
